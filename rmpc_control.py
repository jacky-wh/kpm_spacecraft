...
if __name__ == '__main__':
    ...
    Zdata = np.zeros((nx, Ns + 1))
    Zdata[:,0]=dm_to_array(state_0[:n_normal]).reshape(-1,)
    ...
    for i in range(Ns):
        ...
        uuu=uu+k2(np.matrix(getbasis(Xcurrent,n_basis)).reshape(-1, 1)-state_0)
        state_0 = Alift @ state_0 + Blift @ uu
        #真实状态转移
        sn = odeint(dynamics, Xcurrent, [0, dt], args=(uuu,))
        Xcurrent=sn[-1, :]
